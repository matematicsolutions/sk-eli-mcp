"""Lazy runtime manifest - externalizes VOLATILE config (upstream base URL,
citation-grammar version) out of the shipped package. Piloted on de-eli-mcp.

Pulls a tiny static ``sk-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/sk-runtime.json
+ re-run the release workflow) WITHOUT a PyPI release. Falls back to the hardcoded
default in client.py if the asset is missing/unreachable (offline-safe,
...
Opt-out: set ``SK_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "SK_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/sk-eli-mcp/releases/latest/download/sk-runtime.json.gz",
)
_CACHE_NAME = "sk-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "sk-eli-mcp-runtime (+https://github.com/matematicsolutions/sk-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("SK_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "sk-eli"
